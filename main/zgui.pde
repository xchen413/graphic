void keyPressed() 
    ...
    if(key=='v') showCorner=!showCorner;
    if(key=='&') ; 
    if(key=='*') ;    
    if(key=='(') ;
    if(key==')') ;  
    ...
    if(key=='b') {for(int i=0; i<100; i++) {M.smoothenInterior(0.5);M.smoothenInterior(-0.5); }  M.writeVerticesTo(R);}
    if(key=='c') ;//cut point 
...
void mousePressed() 
  ...
  //  //if(!CutEnd && CutStart){CutEnd = true; CutE = Of;println("cut end vertex "+CutE.x+","+CutE.y);}
  //}
  change=true;
  }
